...
assert.match(index,/engine-v7\.js[^\n]+[\s\S]*mode-expansion\.js[^\n]+[\s\S]*app\.js/,'mode expansion must load after engine-v7 and before app boot');
assert.match(app,/songCountLabel/,'setup must render mode song counts natively');
assert.match(app,/movie\/show year/,'screen modes must explain their answer-year basis');
...
assert.match(app,/screenWorkTitle/,'reveal must identify the represented movie or TV show');
assert.match(app,/remixTitle/,'reveal must identify the remix that was played');
