...
const checks=[
  ...['engine.js','engine-v7.js','app-policy.js','app.js','placement-ui.js'].map(file=>[process.execPath,'--check',file]),
  ...['test_engine_v7','test_runtime_catalogue','test_runtime_recovery','test_spotify_session','test_engine_playback_lifecycle','test_year_gap_evidence',
    ...
    'test_bonus_gameplay','test_physical_ready','test_engine_playback_fixes','test_app_policy','test_card_year_overrides','test_security_contract','test_placement_ui'].map(name=>[process.execPath,`scripts/${name}.mjs`]),
