(() => {
  ...
  async function prepareCard(cardId){
    ...
    try{
      const assignKey=`${roundDeck}:${cardId}`;let song=match.assign[assignKey];
      if(!song){song=await E.chooseSong(year,roundDeck,match.used);match.assign[assignKey]=song;saveMatch()}
      ...
      toast(errorText(err));
      if(err?.code==='NO_SPOTIFY_DEVICE'||err?.code==='SPOTIFY_NOT_CONNECTED'||err?.code==='SPOTIFY_REAUTH'){musicModal=true;screen='setup';render();return}
  ...
  async function playCurrent(){
    ...
    }catch(err){playing=false;toast(errorText(err));if(err?.code?.startsWith('SPOTIFY')){musicModal=true;render()}}
  }
